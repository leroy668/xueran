...
export const getNightRoles = (
  ...
  players
    .filter((player) => {
      ...
      return (firstNight ? role.firstNightOrder : role.otherNightOrder) > 0;
    })
    .map((player) => getRole(player.roleId))
    .sort(
      (a, b) =>
        (firstNight ? a.firstNightOrder : a.otherNightOrder) -
        (firstNight ? b.firstNightOrder : b.otherNightOrder),
    );
