...
import {
  ...
  canRegisterAsGood,
  canRegisterAsRole,
  getChefResultOptions,
  getNearestPlayersByTeam,
  getPossibleEvilCounts,
} from "./registration";
import {
  barberDeathNotePrefix,
  hasActiveVigormortisRetainedAbility,
  getNightActions,
  grandmotherGrandchildNotePrefix,
  getPlayerVisibleRoleId,
  getRole,
  getScriptRoles,
  nodashiiPoisonNotePrefix,
...
import {
  ...
  buildPhilosopherFailedAbilityMessage,
...
import {
  ...
  getExecutionLeader as getRulesExecutionLeader,
...
type DayWorkspaceView = "private-chats" | "voting" | "abilities";
...
const minstrelDrunkNotePrefix = "system:minstrel-drunk:";
...
const getNightStatusMarksForPlayers = (
    ...
    for (const note of parsePlayerNotes(player.notes)) {
      ...
      if (
        (note.id.startsWith(roleStateNotePrefix) ||
          note.id.startsWith(minstrelDrunkNotePrefix)) &&
        note.stage &&
        note.stage !== getGameStageLabel("夜晚", round)
      ) {
        continue;
      }
      if (note.id.startsWith(nodashiiPoisonNotePrefix)) continue;
      if (note.id.startsWith(`${roleStateNotePrefix}minstrel`)) continue;
  ...
  const latestPoisonSeatsBySource = new Map<string, number[]>();
  const latestProtectSeatsBySource = new Map<string, number[]>();
  const nodashiiPoisonTargetsBySource = new Map<string, string[]>();

  // 先计算诺-达鲷的持续中毒候选，再按夜序判断投毒者是否能正常行动。
  for (const source of players) {
    if (
      !source.alive ||
      source.roleId !== "nodashii" ||
      ensureMarks(source.id).has("poisoned") ||
      ensureMarks(source.id).has("drunk")
    ) continue;
    const recordedTargets = players.filter((target) =>
      parsePlayerNotes(target.notes).some(
        (note) =>
          !note.resolved &&
          note.id === `${nodashiiPoisonNotePrefix}${source.id}`,
      ),
    );
    const targets = recordedTargets.length
      ? recordedTargets
      : getNearestPlayersByTeam(players, source.id, "镇民", getRole);
    nodashiiPoisonTargetsBySource.set(
      source.id,
      targets.map((target) => target.id),
    );
  }
  const isProvisionallyPoisonedByNodashii = (playerId: string) =>
    [...nodashiiPoisonTargetsBySource.values()].some((targetIds) =>
      targetIds.includes(playerId),
    );

  // 投毒者早于多数角色行动；先应用其本晚最终选择，供后续能力失效判断使用。
  for (const message of skillMessages) {
    if (
      message.round !== round ||
      message.role_id !== "poisoner" ||
      latestPoisonSeatsBySource.has(message.player_id)
    ) continue;
    const actingPlayer = playersById.get(message.player_id);
    if (
      !actingPlayer ||
      (!actingPlayer.alive &&
        !hasActiveVigormortisRetainedAbility(actingPlayer, players)) ||
    ...
    const targetSeat = extractSeatNumbers(body)[0];
    if (targetSeat !== undefined) {
      latestPoisonSeatsBySource.set(message.player_id, [targetSeat]);
      markSeats([targetSeat], "poisoned");
    }
  }

  const activeNodashiiPoisonedPlayerIds = new Set<string>();
  nodashiiPoisonTargetsBySource.forEach((targetIds, sourceId) => {
    const source = playersById.get(sourceId);
    if (
      !source?.alive ||
      source.roleId !== "nodashii" ||
      ensureMarks(sourceId).has("poisoned") ||
      ensureMarks(sourceId).has("drunk")
    ) return;
    targetIds.forEach((targetId) => {
      ensureMarks(targetId).add("poisoned");
      activeNodashiiPoisonedPlayerIds.add(targetId);
    });
  });
  ...
  for (const message of skillMessages) {
    ...
    const actingAbilitySuppressed = Boolean(
      actingPlayer &&
        (ensureMarks(actingPlayer.id).has("poisoned") ||
          ensureMarks(actingPlayer.id).has("drunk") ||
          getPhilosopherAbilityState(actingPlayer)?.hasAbilityEffect === false),
    );
    ...
    if (actingAbilitySuppressed) continue;
    ...
    if (
      ...
      if (seats.length) {
        const previousPukkaSeat =
          message.role_id === "pukka"
            ? Number(body.match(/上一名中毒者\s*(\d+)\s*号/)?.[1])
            : undefined;
        latestPoisonSeatsBySource.set(
          message.player_id,
          previousPukkaSeat === seats[0] ? [] : [seats[0]],
        );
      }
    }
    if (
      ...
      const poisonClause = body
        .split("；")
        .find((clause) => clause.includes("中毒"));
      if (poisonClause) {
        markSeats(extractSeatNumbers(poisonClause), "poisoned");
      }
    }
    if (
      !latestProtectSeatsBySource.has(message.player_id) &&
      (message.role_id === "monk" ||
        message.role_id === "devils-advocate" ||
        /本晚(?:保护|免死)目标/.test(body))
    ) {
      const seats = extractSeatNumbers(body);
      if (seats.length) {
        latestProtectSeatsBySource.set(message.player_id, [seats[0]]);
      }
    }
  }

  latestPoisonSeatsBySource.forEach((seats) => markSeats(seats, "poisoned"));
  latestProtectSeatsBySource.forEach((seats) => markSeats(seats, "protected"));

  // 普卡的上一名中毒者会一直保持中毒，直到本晚普卡行动结算死亡并恢复健康。
  // 因此在本晚尚未记录普卡行动时，仍需把上一晚的目标标记出来。
  const hasCurrentPukkaAction = skillMessages.some(
    (message) => message.round === round && message.role_id === "pukka",
  );
  if (!hasCurrentPukkaAction) {
    const previousPukkaMessage = skillMessages
      .filter((message) => message.round < round && message.role_id === "pukka")
      .sort((left, right) => {
      ...
      .find((message) => Boolean(getRoleSkillMessage(message.body)));
    const previousPukkaBody = previousPukkaMessage
      ? getRoleSkillMessage(previousPukkaMessage.body)
      : null;
    const previousPukkaSeat = previousPukkaBody
      ? extractSeatNumbers(previousPukkaBody)[0]
      : undefined;
    const previousPukka = previousPukkaMessage
      ? playersById.get(previousPukkaMessage.player_id)
      : null;
    if (
      previousPukkaSeat !== undefined &&
      previousPukka?.alive &&
      previousPukka.roleId === "pukka" &&
      !ensureMarks(previousPukka.id).has("poisoned") &&
      !ensureMarks(previousPukka.id).has("drunk")
    ) {
      markSeats([previousPukkaSeat], "poisoned");
    }
  }
    ...
    byPlayerId.set(
      ...
      [...marks]
        ...
        .map((kind) => ({
          ...
          label:
            kind === "poisoned" &&
            activeNodashiiPoisonedPlayerIds.has(player.id)
              ? "诺-达鲷中毒"
              : markMeta[kind].label,
          kind,
        })),
...
const getDisguiseRoleId = (
  ...
  if (
    preferredRole &&
    allowedTeams.includes(preferredRole.team) &&
  ...
  return (
    rolePool.find(
      (role) =>
        allowedTeams.includes(role.team) && !occupiedRoleIds.has(role.id),
    )?.id ?? ""
  );
};

const formatSeatRoleIdentity = (player: Pick<Player, "seat" | "roleId">) =>
...
function GrimoireApp() {
  ...
  useEffect(() => {
    setState((current) => {
      const rolePool = getScriptRoles(current.scriptId);
      let players = current.players;
      for (const player of current.players) {
        if (player.roleId !== "marionette") continue;
        const shownRole = rolePool.find(
          (role) => role.id === player.drunkRoleId,
        );
        if (shownRole?.team === "镇民") continue;
        const drunkRoleId = getDisguiseRoleId(
          players,
          player.id,
          rolePool,
          "",
        ...
        if (drunkRoleId === player.drunkRoleId) continue;
        players = players.map((candidate) =>
          candidate.id === player.id
            ? { ...candidate, drunkRoleId }
            : candidate,
        );
      }
  ...
  const previousDayResolution = useMemo(
    () =>
      state.round > 1
        ? dayResolutions.find(
            (resolution) => resolution.round === state.round - 1,
          )
        : undefined,
    [dayResolutions, state.round],
  );
  const activeDeathTriggeredPlayerIds = useMemo(() => {
    const playerIds = new Set<string>();
    nightMessages.forEach((message) => {
      if (
        message.round === state.round &&
        message.body === ravenkeeperDeathNotice
      ) {
        playerIds.add(message.player_id);
      }
    });
    if (state.round > 1) {
      playerMessages.forEach((message) => {
        ...
        const player = state.players.find(
          (candidate) => candidate.id === message.player_id,
        );
        if (
          message.round === state.round - 1 &&
          choice?.roleId === "moonchild" &&
          player &&
          !player.alive
        ) {
          playerIds.add(message.player_id);
    ...
    state.players.forEach((player) => {
      if (
        !player.alive &&
        player.roleId === "barber" &&
        parsePlayerNotes(player.notes).some(
          (note) => note.id.startsWith(barberDeathNotePrefix) && !note.resolved,
        )
      ) {
    ...
    return playerIds;
  }, [nightMessages, playerMessages, state.players, state.round]);
  const demonKilledPlayerIds = useMemo(() => {
    const playerIds = new Set<string>();
    ...
    nightMessages.forEach((message) => {
      ...
      const actingPlayer = playersById.get(message.player_id);
      if (!actingPlayer || getRole(actingPlayer.roleId).team !== "恶魔") return;
      const body = getRoleSkillMessage(message.body) ?? message.body;
      const attackedSeat = body.match(/本晚攻击目标[：:]\s*(\d+)\s*号/)?.[1];
      const pukkaDeathSeat = body.match(
        /上一名中毒者\s*(\d+)\s*号结算死亡/,
      )?.[1];
      [attackedSeat, pukkaDeathSeat].forEach((seatText) => {
        if (!seatText) return;
        const target = playersBySeat.get(Number(seatText));
        if (target && !target.alive) playerIds.add(target.id);
      });
    });
    return playerIds;
  }, [nightMessages, state.players, state.round]);
  const nightActionContext = useMemo(
    () => ({
      executedPlayerId:
        previousDayResolution?.executed_player_died === false ||
        previousDayResolution?.executed_player_was_alive === false
          ? null
          : previousDayResolution?.executed_player_id ?? null,
      activeDeathTriggeredPlayerIds,
      demonKilledPlayerIds,
    }),
    [
      activeDeathTriggeredPlayerIds,
      demonKilledPlayerIds,
      previousDayResolution?.executed_player_id,
      previousDayResolution?.executed_player_died,
      previousDayResolution?.executed_player_was_alive,
    ],
  );
  const nightActions = useMemo(
    ...
    [isFirstNight, nightActionContext, state.players, state.round, state.scriptId],
  );
  ...
  const abilityDisabledPlayerIds = useMemo(() => {
    const statuses = getNightStatusMarksForPlayers(
      state.players,
      nightMessages,
      state.round,
    );
    return new Set(
      state.players
        .filter((player) =>
          getPhilosopherAbilityState(player)?.hasAbilityEffect === false ||
          (statuses.get(player.id) ?? []).some(
            (mark) => mark.kind === "poisoned" || mark.kind === "drunk",
          ),
        )
        .map((player) => player.id),
    );
  }, [nightMessages, state.players, state.round]);
  const teaLadyProtectionSources = useMemo(
    () =>
      getTeaLadyProtectionSources(
        state.players,
        abilityDisabledPlayerIds,
        (roleId) => getRole(roleId).team,
      ),
    [abilityDisabledPlayerIds, state.players],
  );
  const retainedAbilityPlayerIds = useMemo(
    () =>
      new Set(
        state.players
          .filter((player) =>
            hasActiveVigormortisRetainedAbility(player, state.players),
          )
          .map((player) => player.id),
      ),
    [state.players],
  );
  const devilsAdvocateProtectedPlayerIds = useMemo(
    () =>
      getDevilsAdvocateProtectedPlayerIds(
        state.players,
        nightMessages,
  ...
  const updatePlayer = (id: string, patch: Partial<Player>) => {
    setState((current) => {
      ...
      return {
        ...current,
        players:
          patch.roleId !== undefined || patch.alive !== undefined
            ? reconcilePhilosopherDrunkenness(
                patchedPlayers,
  ...
  const handleSetPlayerAlive = async (playerId: string, alive: boolean) => {
    ...
    if (!alive && teaLadyProtectionSources.has(playerId)) {
      const sources = teaLadyProtectionSources.get(playerId) ?? [];
      setToast(
        `${formatSeat(player.seat)}受到${sources.map((sourceId) => formatSeat(state.players.find((item) => item.id === sourceId)?.seat)).join("、")}茶艺师保护，不能死亡`,
      );
      return false;
    }

    const players = reconcilePhilosopherDrunkenness(
      state.players.map((item) =>
        item.id === playerId
          ? {
              ...
              alive,
              notes:
                !alive && item.roleId === "barber"
                  ? serializePlayerNotes([
                      {
                        id: `${barberDeathNotePrefix}${state.round}`,
                        body: "理发师已死亡，等待恶魔决定是否交换角色",
                        createdAt: new Date().toISOString(),
                        stage: getGameStageLabel(state.phase, state.round),
                      },
                      ...parsePlayerNotes(item.notes).filter(
                        (note) => note.id !== `${barberDeathNotePrefix}${state.round}`,
                      ),
                    ])
                  : alive && item.roleId === "barber"
                    ? serializePlayerNotes(
                        parsePlayerNotes(item.notes).map((note) =>
                          note.id.startsWith(barberDeathNotePrefix) &&
                          !note.resolved
                            ? { ...note, resolved: true }
                            : note,
                        ),
  ...
  const handleFinalizeExecution = async () => {
    ...
    setRoomBusy(true);
    try {
      const leader = getRulesExecutionLeader(
        nominations.filter((nomination) => nomination.round === state.round),
      );
      const predictedPlayer = leader
        ? state.players.find((player) => player.id === leader.nomineePlayerId)
        : null;
      const protectionReasons = predictedPlayer
        ? [
            ...(teaLadyProtectionSources.has(predictedPlayer.id)
              ? ["茶艺师保护"]
              : []),
            ...(devilsAdvocateProtectedPlayerIds.has(predictedPlayer.id)
              ? ["魔鬼代言人保护"]
              : []),
          ]
        : [];
      ...
      const executedPlayer = result.executed_player_id
        ...
        : null;
      if (executedPlayer?.alive && result.executed_player_died) {
        const activeMinstrels = state.players.filter(
          (player) =>
            player.alive &&
            player.roleId === "minstrel" &&
            !abilityDisabledPlayerIds.has(player.id),
        );
        const minstrelTriggered =
          getRole(executedPlayer.roleId).team === "爪牙" &&
          activeMinstrels.length > 0;
        const drunkStage = getGameStageLabel("夜晚", state.round + 1);
        const nextPlayers = reconcilePhilosopherDrunkenness(
          state.players.map((player) => {
            const notes = parsePlayerNotes(player.notes);
            const deathNotes =
              player.id === executedPlayer.id && player.roleId === "barber"
                ? [
                    {
                      id: `${barberDeathNotePrefix}${state.round}`,
                      body: "理发师已死亡，等待恶魔决定是否交换角色",
                      createdAt: new Date().toISOString(),
                      stage: getGameStageLabel(state.phase, state.round),
                    } satisfies PlayerNoteEntry,
                  ]
                : [];
            const minstrelNotes = minstrelTriggered && player.roleId !== "minstrel"
              ? activeMinstrels.map(
                  (minstrel) =>
                    ({
                      id: `${minstrelDrunkNotePrefix}${minstrel.id}:${state.round}`,
                      body: "吟游诗人能力：醉酒至明天黄昏",
                      createdAt: new Date().toISOString(),
                      stage: drunkStage,
                    }) satisfies PlayerNoteEntry,
              ...
              : [];
            const replacedNoteIds = new Set(
              [...deathNotes, ...minstrelNotes].map((note) => note.id),
            );
            return {
              ...player,
              alive: player.id === executedPlayer.id ? false : player.alive,
              notes: serializePlayerNotes([
                ...deathNotes,
                ...minstrelNotes,
                ...notes.filter((note) => !replacedNoteIds.has(note.id)),
              ]),
            };
          }),
          (roleId) => getRole(roleId).name,
        ...
        await syncRoom(room.id, nextState);
        const visibleRoleId =
          getPhilosopherAbilityRoleId(executedPlayer) ||
          getPlayerVisibleRoleId(executedPlayer.roleId, executedPlayer.drunkRoleId);
        const notice = getDeathTriggeredAbilityNotice(visibleRoleId);
        const roomPlayer = roomPlayers.find((item) => item.id === executedPlayer.id);
        if (notice && roomPlayer?.is_claimed) {
          await handleSendNightMessage({
      ...
      setToast(
        executedPlayer
          ? result.executed_player_died
            ? `${formatSeat(executedPlayer.seat)}已被处决并死亡`
            : `${formatSeat(executedPlayer.seat)}已被处决但因${protectionReasons.join("与") || "能力"}没有死亡`
          : "今日投票结束，无人被处决",
      );
    } catch {
      ...
      <main className="workspace">
        ...
        {activeTab === "night" ? (
          <NightPanel
            ...
            nightActionContext={nightActionContext}
            onUpdate={update}
          ...
          <div className="day-workspace">
            ...
            <div className="day-workspace-toolbar">
              ...
              >
                ...
                <button
                  className={dayWorkspaceView === "abilities" ? "active" : ""}
                  role="tab"
                  aria-selected={dayWorkspaceView === "abilities"}
                  onClick={() => setDayWorkspaceView("abilities")}
                >
                  <Target size={15} />
                  白天技能
                </button>
...
function GrimoirePanel({
  ...
  const statusMarksByPlayerId = useMemo(
    () =>
              ...
              <div className="grimoire-table">
                ...
                {state.players.map((player, index) => {
                  ...
                  const redHerring = isFortuneTellerRedHerring(player);
                   ...
                   const activeStatusMarks = (
                     statusMarksByPlayerId.get(player.id) ?? []
                   ).filter((mark) => mark.kind !== "dead");
                   const poisonedStatus = activeStatusMarks.find(
                     (mark) => mark.kind === "poisoned",
                   );
                   const isPoisoned = Boolean(poisonedStatus);
                   const cardNoteCount =
                  ...
                  return (
                    <button
                      ...
                      aria-label={`${formatSeat(player.seat)}，${getDisplayName(player)}，${role.name}，${player.alive ? "存活" : "死亡"}${activeStatusMarks.length ? `，当前状态：${activeStatusMarks.map((mark) => mark.label).join("、")}` : ""}${redHerring ? `，${fortuneTellerRedHerringNoteBody}` : ""}${roleStateHistory.length ? `，角色状态：${roleStateHistory.map((entry) => `${entry.stage ?? "阶段未记录"} ${entry.body}`).join("；")}` : ""}${infoPreview ? `，已传达信息：${infoPreview}` : ""}`}
                      aria-pressed={isSelected}
                    >
                      {poisonedStatus ? (
                        <span
                          className="table-seat-status-badge is-poisoned"
                          title={poisonedStatus.label}
                        >
                          <Biohazard size={10} />
                          中毒
                        </span>
                      ...
                      <span className="table-role-name">
                       ...
                       {cardNoteCount > 0 ? (
                         <span className="table-card-notes">
                           {activeStatusMarks.length ? (
                             <span
                               className={`table-card-note is-current-status${isPoisoned ? " is-poisoned" : ""}`}
                               title={`当前状态：${activeStatusMarks.map((mark) => mark.label).join("、")}`}
                             >
                               <strong>当前状态</strong>
                               <span>
                                 {activeStatusMarks
...
function HostDayAbilitiesPanel({
  state,
  room,
  roomPlayers,
  playerMessages,
  ...
  const dayRoles = new Set(["slayer", "moonchild", "klutz"]);
  const entries = state.players
    .map((player) => {
      const roleId =
        getPhilosopherAbilityRoleId(player) ||
        getPlayerVisibleRoleId(player.roleId, player.drunkRoleId);
      return { player, roleId, role: getRole(roleId) };
    })
    .filter(({ roleId }) => dayRoles.has(roleId));

  const latestChoiceFor = (playerId: string, roleId: string) =>
    [...playerMessages]
      .filter(
        (message) =>
          message.player_id === playerId && message.round === state.round,
      )
      .sort(
        (left, right) =>
          new Date(right.created_at).getTime() -
      ...
      .find((choice) => choice?.roleId === roleId) ?? null;

  const resolveChoice = async (player: Player, roleId: string) => {
    const choice = latestChoiceFor(player.id, roleId);
    const target = state.players.find(
      (candidate) => candidate.id === choice?.playerIds[0],
    );
    if (!choice || !target || !room || busy) return;
    const targetRole = getRole(target.roleId);
    const shownAbilityHasEffect =
      player.roleId !== "drunk" &&
      player.roleId !== "marionette" &&
      !abilityDisabledPlayerIds.has(player.id);
    setBusy(true);
    ...
    try {
      if (roleId === "slayer") {
        const isDemon = targetRole.team === "恶魔";
        const canRegisterAsDemon = target.roleId === "recluse";
        if (
          !window.confirm(
            `${choice.summary}\n实际身份：${targetRole.name}\n${
              isDemon
                ? "应被杀手能力击杀。"
                : canRegisterAsDemon
                  ? "陌客可登记为恶魔，由上帝决定是否击杀。"
        ...
        const shouldKill = shownAbilityHasEffect &&
          (isDemon ||
            (canRegisterAsDemon &&
              window.confirm("是否将陌客登记为恶魔并标记死亡？选择取消则记录为未命中。")));
        await onSendMessage({
          playerId: player.id,
          roleId,
          body: buildRoleSkillMessage(
            `${choice.summary}：${shouldKill ? "命中并死亡" : shownAbilityHasEffect ? "未命中恶魔" : "能力失效，目标未死亡"}`,
          ),
        });
        if (shouldKill && target.alive) {
          await onSetPlayerAlive(target.id, false);
        }
        setNotice(
          shouldKill
            ? `${formatSeat(target.seat)}已因杀手能力死亡`
            : shownAbilityHasEffect
              ? `${formatSeat(target.seat)}不应因杀手能力死亡`
              : `${formatSeat(player.seat)}当前能力失效，目标未死亡`,
        );
        return;
      }
      const actualGood = ["镇民", "外来者"].includes(targetRole.team);
      const canBeGood = canRegisterAsGood(target.roleId, targetRole.team);
      const canBeEvil = canRegisterAsEvil(target.roleId, targetRole.team);
      const result = roleId === "moonchild"
        ? !shownAbilityHasEffect
          ? "能力失效，目标今晚不会因此死亡"
          : canBeGood
          ? "目标可登记为善良，今晚应死亡"
        ...
        : !shownAbilityHasEffect
          ...
          ? "目标可登记为邪恶，呆瓜阵营可能落败"
          : "目标不能登记为邪恶，游戏继续";
      if (!window.confirm(`${choice.summary}\n实际阵营：${actualGood ? "善良" : "邪恶"}\n规则判断：${result}\n${roleId === "moonchild" ? "只确认选择，不向月之子透露目标阵营；能力将在夜晚结算。" : "确认发送给玩家？"}`)) return;
      await onSendMessage({
        playerId: player.id,
        roleId,
        body: buildRoleSkillMessage(
          roleId === "moonchild"
            ? `${choice.summary}：选择已记录，等待夜晚结算`
            : `${choice.summary}：${result}`,
        ),
      });
      setNotice(result);
    } finally {
  ...
  return (
    <section className="host-voting-page">
      <div className="panel-heading host-voting-heading">
        <div><p className="eyebrow">DAY ABILITIES · 第 {state.round} 天</p><h2>白天角色技能</h2></div>
        <Target size={22} />
      </div>
      {entries.length ? (
        <div className="nomination-list host">
          {entries.map(({ player, roleId, role }) => {
            const choice = latestChoiceFor(player.id, roleId);
            const used = nightMessages.some(
              (message) =>
                message.player_id === player.id &&
                message.role_id === roleId &&
                Boolean(getRoleSkillMessage(message.body)),
            );
            const claimed = roomPlayersById.get(player.id)?.is_claimed;
            return (
              <article className="nomination-card closed" key={`${player.id}-${roleId}`}>
                <div className="nomination-route">
                  <span>{formatSeat(player.seat)} · {role.name}</span>
                  <strong>{choice?.summary ?? "等待玩家提交选择"}</strong>
                </div>
                <button
                  className="primary-button"
                  disabled={!choice || !claimed || busy || used}
                  onClick={() => void resolveChoice(player, roleId)}
                >
                  <Check size={15} />{used ? "本日已结算" : "校验并结算"}
                </button>
              </article>
            );
          })}
        </div>
      ) : <div className="voting-inline-notice">当前在场角色没有需要在白天由上帝结算的主动技能</div>}
      {notice ? <div className="voting-inline-notice">{notice}</div> : null}
    </section>
  );
}
function NightPanel({
  ...
  nightActionContext: NonNullable<Parameters<typeof getNightActions>[4]>;
  onUpdate: (patch: Partial<GameState>) => void;
  onSetPlayerAlive: (playerId: string, alive: boolean) => Promise<boolean>;
  ...
  const [scarletDemonRoleId, setScarletDemonRoleId] = useState("");
  const [vigormortisKillConfirmed, setVigormortisKillConfirmed] =
    useState(false);
  const [registerSpyAsTownsfolkForAdjacency, setRegisterSpyAsTownsfolkForAdjacency] =
    useState(false);
  const [godfatherOutsiderRoleIds, setGodfatherOutsiderRoleIds] = useState<
    string[]
  >([]);
  ...
  const isMinionInfoAction = currentAction?.id === "minion-info";
  const isDemonInfoAction = currentAction?.id === "demon-info";
  const currentStageLabel = getGameStageLabel(state.phase, state.round);
  const isNodashiiFirstNight =
    currentRole?.id === "nodashii" && state.round <= 1;
  ...
  const nightAbilityDisabledPlayerIds = useMemo(
    () =>
      new Set(
        state.players
          .filter(
            (player) =>
              getPhilosopherAbilityState(player)?.hasAbilityEffect === false ||
              (nightStatusByPlayerId.get(player.id) ?? []).some(
                (mark) => mark.kind === "poisoned" || mark.kind === "drunk",
              ),
          )
          .map((player) => player.id),
      ),
    [nightStatusByPlayerId, state.players],
  );
  const vortoxActive = isVortoxActive(
    state.players,
    nightAbilityDisabledPlayerIds,
  );
  const poisonedPlayerStatuses = state.players.flatMap((player) => {
    const mark = (nightStatusByPlayerId.get(player.id) ?? []).find(
      (status) => status.kind === "poisoned",
    );
  ...
  const firstNightDemonPlayers = useMemo(
    () =>
      state.players
        .filter((player) => getRole(player.roleId).team === "恶魔")
        .sort((left, right) => left.seat - right.seat),
    [state.players],
  );
  const firstNightMinionPlayers = useMemo(
    () =>
      state.players
        .filter((player) => getRole(player.roleId).team === "爪牙")
        .sort((left, right) => left.seat - right.seat),
    [state.players],
  );
  const firstNightAwakeMinionPlayers = useMemo(
    () =>
      firstNightMinionPlayers.filter(
        (player) => player.roleId !== "marionette",
      ),
  ...
  const firstNightLunaticPlayers = useMemo(
    () => state.players.filter((player) => player.roleId === "lunatic"),
    [state.players],
  );
  ...
  const demonMinionInfoBody = (firstNightMagicianPlayers.length
    ...
    (firstNightLunaticPlayers.length
      ? `；疯子身份：${firstNightLunaticPlayers.map((player) => formatSeat(player.seat)).join("、")}`
      : "");
  const selectedPlayer = rolePlayers.find(
    (player) => player.id === targetPlayerId,
  );
  const previousDevilsAdvocateTargetId = useMemo(
    () =>
      currentRole?.id === "devils-advocate" && selectedPlayer
        ? getPreviousRoleTargetPlayerId(
            state.players,
            nightMessages,
            selectedPlayer.id,
        ...
        : null,
    [currentRole?.id, nightMessages, selectedPlayer, state.players, state.round],
  );
  ...
  const philosopherAbilityJustAcquired = Boolean(
    currentAction?.isPhilosopherAbility &&
      selectedPlayer &&
      getPhilosopherAbilityState(selectedPlayer)?.note.stage === currentStageLabel,
  );
  ...
  const impSuccessorCandidates = useMemo(
    () => {
      ...
      const activeScarletWoman = candidates.find((player) => {
        if (player.roleId !== "scarlet-woman") return false;
        return !(nightStatusByPlayerId.get(player.id) ?? []).some(
          (mark) => mark.kind === "poisoned" || mark.kind === "drunk",
        );
      });
      return activeScarletWoman &&
        state.players.filter((player) => player.alive).length >= 5
        ? [activeScarletWoman]
        : candidates;
    },
    [nightStatusByPlayerId, selectedPlayer?.id, state.players],
  );
  const scriptDemonRoles = useMemo(
    () => getScriptRoles(state.scriptId).filter((role) => role.team === "恶魔"),
    [state.scriptId],
  );
  const deadDemonPlayers = useMemo(
    () =>
      state.players.filter(
        (player) => !player.alive && getRole(player.roleId).team === "恶魔",
      ),
    [state.players],
  );
  ...
  const currentSkillDefinition =
    currentRole && !isDemonInfoAction && !isMinionInfoAction
      ? getTroubleBrewingSkill(currentRole.id)
      : null;
  const firstNightTargetSkillLocked = Boolean(
    state.phase === "夜晚" &&
      state.round <= 1 &&
      currentSkillDefinition?.playerChoice &&
      !currentSkillDefinition.playerChoice.allowFirstNight,
  );
  ...
  const demonMinionInfoAlreadySent = Boolean(
    selectedPlayer &&
      nightMessages.some(
        (message) =>
          message.player_id === selectedPlayer.id &&
          message.round === state.round &&
          message.body === demonMinionInfoBody,
      ),
  );
  const demonInfoAlreadySent =
    demonBluffsAlreadySent && demonMinionInfoAlreadySent;
  ...
  const singleTargetCandidates = useMemo(() => {
    ...
    const requiresAlive = Boolean(
      playerChoice?.aliveOnly || currentRole.id === "gossip",
    );
    const executedPlayer = state.players.find(
      (player) => player.id === nightActionContext.executedPlayerId,
    );
    const pool = currentRole.id === "undertaker"
      ? executedPlayer
        ? [executedPlayer]
        : []
      : state.players;
    return pool.filter((player) => {
      ...
      if (
        ...
        player.id === previousDevilsAdvocateTargetId
      ) return false;
      return true;
    });
  }, [currentRole, nightActionContext.executedPlayerId, previousDevilsAdvocateTargetId, state.players, targetPlayerId]);
  const grandchildCandidates = useMemo(
    () =>
      state.players.filter((player) => {
        const role = getRole(player.roleId);
        return canRegisterAsGood(player.roleId, role.team);
      }),
    [state.players],
  );
  const scriptOutsiderRoles = useMemo(
    () => getScriptRoles(state.scriptId).filter((role) => role.team === "外来者"),
    [state.scriptId],
  );
  const actualOutsiderRoleIds = useMemo(
    () =>
      [...new Set(
        state.players
          .filter((player) => getRole(player.roleId).team === "外来者")
  ...
  const chefResultOptions = useMemo(
    () => getChefResultOptions(state.players, getRole),
    [state.players],
  );
  ...
  const empathResultOptions = useMemo(
    () =>
      getPossibleEvilCounts(empathNeighbors, (player) => ({
        roleId: player.roleId,
        team: getRole(player.roleId).team,
      })),
    [empathNeighbors],
  );
  ...
  useEffect(() => {
    if (currentRole?.id !== "scarlet-woman") return;
    const preferredRoleId = deadDemonPlayers.at(-1)?.roleId;
    if (preferredRoleId && preferredRoleId !== scarletDemonRoleId) {
      setScarletDemonRoleId(preferredRoleId);
      return;
    }
    if (!scriptDemonRoles.some((role) => role.id === scarletDemonRoleId)) {
      setScarletDemonRoleId(scriptDemonRoles[0]?.id ?? "");
    }
  }, [
    currentRole?.id,
    deadDemonPlayers,
  ...
  const previousPukkaTarget = useMemo(() => {
    if (currentRole?.id !== "pukka" || !selectedPlayer) return null;
    const previousMessage = [...nightMessages]
      .filter(
        (message) =>
          message.player_id === selectedPlayer.id &&
          message.role_id === "pukka" &&
    ...
    const previousBody = previousMessage
      ? getRoleSkillMessage(previousMessage.body)
      : null;
    const previousSeat = previousBody
      ? extractSeatNumbers(previousBody)[0]
      : undefined;
    return previousSeat === undefined
      ? null
      : state.players.find((player) => player.seat === previousSeat) ?? null;
  }, [currentRole?.id, nightMessages, selectedPlayer, state.players, state.round]);
  const nodashiiPoisonedPlayers = useMemo(() => {
    if (currentRole?.id !== "nodashii" || !selectedPlayer) return [];
    return getNearestPlayersByTeam(
      state.players,
      selectedPlayer.id,
      "镇民",
      getRole,
      registerSpyAsTownsfolkForAdjacency,
    );
  }, [
    currentRole?.id,
    registerSpyAsTownsfolkForAdjacency,
  ...
  const persistNodashiiPoisonTargets = () => {
    if (currentRole?.id !== "nodashii" || !selectedPlayer) return;
    const noteId = `${nodashiiPoisonNotePrefix}${selectedPlayer.id}`;
    const targetIds = new Set(
      nodashiiPoisonedPlayers.map((player) => player.id),
    );
  ...
  const vigormortisPoisonCandidates = useMemo(() => {
    if (currentRole?.id !== "vigormortis" || !singleSkillTargetId) return [];
    const target = state.players.find(
  ...
  const latestPlayerSkillChoiceEntry = useMemo(() => {
  ...
  const latestPlayerSkillChoice = latestPlayerSkillChoiceEntry?.choice ?? null;
  const latestPlayerSkillChoiceMessageId =
    latestPlayerSkillChoiceEntry?.message.id ?? "";
  const previousDayRound = Math.max(1, state.round - 1);
  const townCrierNominators = useMemo(
    () =>
      nominations
        .filter((nomination) => nomination.round === previousDayRound)
        .map((nomination) =>
          state.players.find(
            (player) => player.id === nomination.nominator_player_id,
          ),
        )
        .filter((player): player is Player => Boolean(player)),
    [nominations, previousDayRound, state.players],
  );
  const townCrierActualYes = townCrierNominators.some(
    (player) => getRole(player.roleId).team === "爪牙",
  );
  const townCrierResultOptions = useMemo(() => {
    const options = new Set([townCrierActualYes]);
    if (townCrierNominators.some((player) => player.roleId === "recluse")) {
      options.add(true);
    }
    if (
      townCrierNominators.some(
        (player) => player.roleId === "spy" || player.roleId === "marionette",
      )
    ) {
      options.add(false);
    }
    return options;
  }, [townCrierActualYes, townCrierNominators]);
  const barberDemon = useMemo(
    () =>
      state.players.find(
        (player) => player.alive && getRole(player.roleId).team === "恶魔",
      ) ?? null,
  ...
  const barberSwapCandidates = useMemo(
    () =>
      state.players.filter(
        (player) =>
          getRole(player.roleId).team !== "恶魔" ||
          player.id === barberDemon?.id,
      ),
    [barberDemon?.id, state.players],
  );
  const jugglerGuessRows = useMemo(() => {
    ...
    return (latestPlayerSkillChoice?.guesses ?? []).map((guess) => {
      ...
      const correct = Boolean(target && target.roleId === guess.roleId);
      const registrationCompatible = Boolean(
        target && canRegisterAsRole(target.roleId, guess.roleId, getRole),
      );
      const flexible = Boolean(
        target && ["recluse", "spy"].includes(target.roleId),
      );
      return {
        target,
        guessedRole,
  ...
  const jugglerResultOptions = useMemo(() => {
    let counts = new Set([0]);
    jugglerGuessRows.forEach((guess) => {
      const options = guess.registrationCompatible
        ? guess.flexible
          ? [0, 1]
          : [1]
        : [0];
      counts = new Set(
        [...counts].flatMap((count) =>
          options.map((matched) => count + matched),
        ),
      );
    });
  ...
  const pairSkillRole =
    ...
    currentRole?.id === "seamstress";
  const pairSkillCandidates =
    currentRole?.id === "chambermaid" || currentRole?.id === "seamstress"
      ? singleTargetCandidates
      : fortuneTellerCandidates;
  const pairSkillCandidateIds = pairSkillCandidates
    .map((player) => player.id)
    .join("|");
  const acceptsSubmittedPair =
    currentRole?.id === "fortune-teller" ||
    currentRole?.id === "chambermaid" ||
    currentRole?.id === "seamstress";
  const submittedPairFirst = acceptsSubmittedPair
    ? latestPlayerSkillChoice?.playerIds[0] ?? ""
    : "";
  const submittedPairSecond = acceptsSubmittedPair
    ? latestPlayerSkillChoice?.playerIds[1] ?? ""
    : "";

  useEffect(() => {
    if (!pairSkillRole) return;
    const candidateIds = pairSkillCandidateIds
      ? pairSkillCandidateIds.split("|")
      : [];
    const candidateIdSet = new Set(candidateIds);
    const nextFirst = submittedPairFirst && candidateIdSet.has(submittedPairFirst)
      ? submittedPairFirst
      : candidateIds[0] ?? "";
    const nextSecond =
      submittedPairSecond &&
      candidateIdSet.has(submittedPairSecond) &&
      submittedPairSecond !== nextFirst
        ? submittedPairSecond
        : candidateIds.find((playerId) => playerId !== nextFirst) ?? "";
    setSkillTargets({ first: nextFirst, second: nextSecond });
  }, [
    currentRole?.id,
    latestPlayerSkillChoiceMessageId,
    pairSkillCandidateIds,
    pairSkillRole,
    submittedPairFirst,
    submittedPairSecond,
    targetPlayerId,
  ...
  useEffect(() => {
    if (!pairSkillRole) return;
    const candidateIds = new Set(pairSkillCandidates.map((player) => player.id));
    const first = candidateIds.has(skillTargets.first)
      ? skillTargets.first
      : pairSkillCandidates[0]?.id ?? "";
    const second =
      candidateIds.has(skillTargets.second) && skillTargets.second !== first
        ? skillTargets.second
        : pairSkillCandidates.find((player) => player.id !== first)?.id ?? "";
    if (first !== skillTargets.first || second !== skillTargets.second) {
  ...
  useEffect(() => {
    if (currentRole?.id !== "barber") return;
    const candidateIds = new Set(barberSwapCandidates.map((player) => player.id));
    const first = candidateIds.has(skillTargets.first)
      ? skillTargets.first
      : barberSwapCandidates[0]?.id ?? "";
    const second =
      candidateIds.has(skillTargets.second) && skillTargets.second !== first
        ? skillTargets.second
        : barberSwapCandidates.find((player) => player.id !== first)?.id ?? "";
    if (first !== skillTargets.first || second !== skillTargets.second) {
      setSkillTargets({ first, second });
    }
  }, [
    barberSwapCandidates,
    currentRole?.id,
    skillTargets.first,
    skillTargets.second,
  ]);

  const singleTargetCandidateIds = singleTargetCandidates
    .map((player) => player.id)
    .join("|");
  const submittedSingleTargetId =
    latestPlayerSkillChoice?.playerIds[0] ?? "";
  useEffect(() => {
    const candidateIds = singleTargetCandidateIds
      ? singleTargetCandidateIds.split("|")
      : [];
    const candidateIdSet = new Set(candidateIds);
    setSingleSkillTargetId(
      submittedSingleTargetId && candidateIdSet.has(submittedSingleTargetId)
        ? submittedSingleTargetId
        : candidateIds[0] ?? "",
    );
  }, [
    currentRole?.id,
    latestPlayerSkillChoiceMessageId,
    singleTargetCandidateIds,
    submittedSingleTargetId,
    targetPlayerId,
  ]);
  useEffect(() => {
    const candidateIds = new Set(singleTargetCandidates.map((player) => player.id));
    const nextTarget = candidateIds.has(singleSkillTargetId)
      ? singleSkillTargetId
  ...
  useEffect(() => {
    ...
    if (!target) return;
    if (currentRole?.id === "grandmother" && target.roleId === "spy") {
      const shownRoleIsGood = ["镇民", "外来者"].includes(
        getRole(revealedSkillRoleId).team,
      );
      if (!shownRoleIsGood) {
        setRevealedSkillRoleId(
          getScriptRoles(state.scriptId).find((role) =>
            ["镇民", "外来者"].includes(role.team),
          )?.id ?? "",
        );
      }
      return;
    }
    setRevealedSkillRoleId(target.roleId);
  }, [
    currentRole?.id,
    revealedSkillRoleId,
    singleSkillTargetId,
  ...
  useEffect(() => {
    ...
    if (
      !vigormortisPoisonCandidates.some(
        (player) => player.id === skillTargets.second,
      )
    ) {
      setSkillTargets((current) => ({
        ...current,
        second: vigormortisPoisonCandidates[0]?.id ?? "",
      }));
    }
  }, [
    currentRole?.id,
    skillTargets.second,
    vigormortisPoisonCandidates,
  ]);

  useEffect(() => {
    setVigormortisKillConfirmed(false);
  }, [currentRole?.id, singleSkillTargetId, state.round]);

  useEffect(() => {
    setRegisterSpyAsTownsfolkForAdjacency(false);
  }, [currentRole?.id, state.round]);

  useEffect(() => {
    if (currentRole?.id !== "godfather" || state.round > 1) return;
    setGodfatherOutsiderRoleIds(actualOutsiderRoleIds);
  }, [actualOutsiderRoleIds, currentRole?.id, state.round]);
  ...
  const generateAutomaticPairSkill = useCallback(() => {
    ...
    if (
      (roleId === "librarian" || roleId === "investigator") &&
  ...
  const vortoxInformationRoleIds = new Set([
    "washerwoman",
    "librarian",
    "investigator",
    "chef",
  ...
  const confirmSkillResult = ({
    ...
    sentMatchesStrictBaseline?: boolean | null;
    baseline: string;
    caveats?: string[];
  }) => {
    const recipient = selectedPlayer
      ...
      : selectedPlayerName;
    const vortoxRequiresFalseInformation = Boolean(
      vortoxActive &&
      currentRole &&
      vortoxInformationRoleIds.has(currentRole.id) &&
      selectedAbilityIssueLabels.length === 0,
    );
    const informationCompliance = getInformationResultCompliance({
      vortoxActive: vortoxRequiresFalseInformation,
      registrationCompatible: sentMatchesBaseline,
      matchesStrictTruth:
        sentMatchesStrictBaseline === undefined
          ? sentMatchesBaseline
          : sentMatchesStrictBaseline,
    });
    const resultMatchesRules = informationCompliance !== false;
    const judgement = selectedAbilityIssueLabels.length
      ? "辅助判断：结果可自由裁定（该角色能力当前可能失效）"
      : informationCompliance === null
        ? "辅助判断：涡流生效，请按本晚实际唤醒或登记结果确保信息为假"
      : resultMatchesRules
        ? vortoxRequiresFalseInformation
          ? "辅助判断：涡流生效，发送结果与真实基准不符，符合规则"
          : "辅助判断：符合当前真实身份基准"
        : vortoxRequiresFalseInformation
          ? "辅助判断：涡流生效，镇民不能获得真实信息"
          : "辅助判断：与当前真实身份基准不符";
    const issueLines = selectedAbilityIssueLabels.length
      ? [`能力状态：${selectedAbilityIssueLabels.join("、")}`]
      : [];
    const effectiveCaveats = [
      ...(vortoxRequiresFalseInformation
        ? ["存活且能力正常的涡流要求镇民信息必定为假"]
        : []),
      ...caveats,
    ];
    const caveatLines = effectiveCaveats.length
      ? ["规则例外：" + effectiveCaveats.join("；")]
      : [];
    return new Promise<boolean>((resolve) => {
      setSkillResultConfirmation({
        ...
        caveatLines,
        mismatch:
          selectedAbilityIssueLabels.length === 0 && !resultMatchesRules,
        resolve,
      });
    });
  };
  ...
  const canSend =
    ...
    !sending;
  const currentSkillAlreadyResolved = Boolean(
    currentSkillDefinition?.playerChoice?.oneUse &&
      selectedPlayer &&
      nightMessages.some((message) => {
        if (
          message.player_id !== selectedPlayer.id ||
          message.role_id !== currentRole?.id
        ) return false;
        const skillBody = getRoleSkillMessage(message.body);
        return Boolean(skillBody && !skillBody.includes("等待夜晚结算"));
      }),
  );
  const currentRoundRamsaySkillResolved = Boolean(
    selectedPlayer &&
      ["town-crier", "gossip", "barber"].includes(currentRole?.id ?? "") &&
      nightMessages.some(
        (message) =>
          message.player_id === selectedPlayer.id &&
          message.role_id === currentRole?.id &&
  ...
  const canUseSkill =
    ...
    Boolean(selectedRoomPlayer?.is_claimed || currentRole?.id === "barber") &&
    !currentSkillAlreadyResolved &&
    !currentRoundRamsaySkillResolved &&
    !sending;
  const canResolveBarber =
    currentRole?.id === "barber" &&
    state.phase === "夜晚" &&
    Boolean(currentAction?.canAct) &&
    !currentRoundRamsaySkillResolved &&
    !sending;
  const canUseSingleTargetSkill =
    canUseSkill && !firstNightTargetSkillLocked;
  ...
  const sendDemonBluffs = async () => {
    ...
    try {
      ...
      if (!demonMinionInfoAlreadySent) {
        await onSendMessage({
          playerId: selectedPlayer.id,
          roleId: currentRole.id,
          body: demonMinionInfoBody,
        });
      }
    } catch {
      setSendError(
  ...
  const sendMinionInfo = async () => {
    if (!currentRole || !selectedPlayer || !isMinionInfoAction || !room || sending) {
      return;
    }
    const recipients = firstNightAwakeMinionPlayers.filter(
      (player) => roomPlayersById.get(player.id)?.is_claimed,
    );
    if (!recipients.length) {
      setSendError("爪牙玩家入座后才可发送阵营信息");
      return;
    }
    for (const recipient of recipients) {
      const body = [
        `恶魔身份：${demonIdentitySummary}`,
        `爪牙同伴身份：${
          firstNightAwakeMinionPlayers
            .filter((player) => player.id !== recipient.id)
            .map(formatSeatRoleIdentity)
            .join("、") || "无"
        }`,
      ].join("；");
  ...
  const confirmPhilosopherAbility = async () => {
    ...
    const hasAbilityEffect = selectedAbilityIssueLabels.length === 0;
    const sent = await submitSkill(
      hasAbilityEffect
        ? buildPhilosopherAbilityMessage(chosenRole.name)
        : buildPhilosopherFailedAbilityMessage(chosenRole.name),
    );
    if (!sent) return;
  ...
  const noPairRoleInPlay = Boolean(
    librarianNoOutsider &&
      (currentRole?.id === "librarian" || currentRole?.id === "investigator"),
  );
  const sendPairRoleSkill = async () => {
    ...
    const body =
      currentRole.id === "washerwoman"
        ...
        : currentRole.id === "librarian" || currentRole.id === "investigator"
          ? noPairRoleInPlay
            ? `本局没有${currentRole.id === "librarian" ? "外来者" : "爪牙"}`
            : `${first} 和 ${second} 中，有一人是${selectedInfoRole.name}`
          : `${first} 和 ${second} 中，有一人是${selectedInfoRole.name}`;
    ...
    const actualMinions = state.players.filter(
      (player) => getRole(player.roleId).team === "爪牙",
    );
    const sentMatchesBaseline =
      noPairRoleInPlay
        ? currentRole.id === "librarian"
          ? actualOutsiders.every((player) => player.roleId === "recluse")
          : actualMinions.every((player) => player.roleId === "spy")
        : targetPlayers.some((player) =>
            canRegisterAsRole(player.roleId, skillRoleId, getRole),
          );
    const sentMatchesStrictBaseline = noPairRoleInPlay
      ? currentRole.id === "librarian"
        ? actualOutsiders.length === 0
        : actualMinions.length === 0
      ...
      noPairRoleInPlay
        ? currentRole.id === "librarian"
          ? actualOutsiders.length
            ...
            : "场上确实没有外来者"
          : actualMinions.length
            ? `场上实际有爪牙：${actualMinions.map((player) => `${formatSeat(player.seat)} ${getRole(player.roleId).name}`).join("、")}`
            : "场上确实没有爪牙"
    ...
    const hasRelevantRegistrationRole =
      currentRole.id === "investigator"
        ? noPairRoleInPlay
          ? actualMinions.some((player) => player.roleId === "spy")
  ...
  const sendChefResult = async () => {
    ...
    if (!(await confirmSkillResult({
      ...
      sentMatchesBaseline: chefResultOptions.includes(chefResult),
      sentMatchesStrictBaseline: chefResult === calculatedChefResult,
      baseline: `按登记规则可为 ${chefResultOptions.join("、")} 对（按真实阵营为 ${calculatedChefResult} 对）`,
  ...
  const sendEmpathResult = async (count: number) => {
    ...
    if (!(await confirmSkillResult({
      ...
      sentMatchesBaseline: empathResultOptions.includes(count),
      sentMatchesStrictBaseline: count === calculatedEmpathResult,
      baseline: `两名存活邻座为${empathNeighbors.map((player) => `${formatSeat(player.seat)} ${getRole(player.roleId).name}`).join("、") || "未找到"}；按登记规则可为 ${empathResultOptions.join("、")}（按真实阵营为 ${calculatedEmpathResult}）`,
      caveats: registrationRoles.length
  ...
  const sendSingleTargetSkill = async () => {
    ...
    if (state.phase !== "夜晚") {
      setSendError("请先进入夜晚阶段再结算夜间能力");
      return;
    }
    if (firstNightTargetSkillLocked) {
      setSendError(`${currentRole.name}首夜不能发动这项能力`);
      return;
    }
    if (
    ...
    if (
      currentRole.id === "devils-advocate" &&
      singleSkillTargetId === previousDevilsAdvocateTargetId
    ) {
      setSendError("魔鬼代言人不能连续两晚选择同一名玩家");
      return;
    }
    ...
    if (
      ...
      !window.confirm(
        impSuccessor
          ...
          : "确认小恶魔自杀？当前没有存活爪牙可以继承，小恶魔死亡后游戏可能结束。",
      )
    ) {
      return;
    }
    const vigormortisTarget = state.players.find(
      (player) => player.id === singleSkillTargetId,
    );
    const vigormortisKillsMinion = Boolean(
      currentRole.id === "vigormortis" &&
      selectedAbilityIssueLabels.length === 0 &&
      vigormortisKillConfirmed &&
      vigormortisTarget &&
      getRole(vigormortisTarget.roleId).team === "爪牙",
    );
    if (vigormortisKillsMinion && !skillTargets.second) {
      setSendError("请选择一名邻近镇民中毒");
      return;
    }
    const adjacencyRegistrationNote = registerSpyAsTownsfolkForAdjacency
      ? "（本次将间谍登记为镇民）"
      : "（本次按真实身份判定）";
    const body =
      currentRole.id === "monk"
            ...
            : currentRole.id === "imp"
              ...
              : currentRole.id === "pukka"
                ? "本晚新中毒目标（先放置）：" +
                  targetLabel +
                  (previousPukkaTarget
                    ? `；随后上一名中毒者${formatSeat(previousPukkaTarget.seat)}结算死亡并恢复健康${previousPukkaTarget.id === singleSkillTargetId ? "（与新目标相同，最终恢复健康）" : ""}`
                    : state.round <= 1
                      ? "；首夜没有上一名中毒者"
                      : "；未找到上一名中毒记录，请上帝手动核对")
                : currentRole.id === "vigormortis"
                  ? "本晚攻击目标：" + targetLabel +
                    (vigormortisKillsMinion
                      ? `；目标爪牙实际死亡并保留能力，${getNightSeatLabel(skillTargets.second)}中毒${adjacencyRegistrationNote}`
                      : getRole(vigormortisTarget?.roleId ?? "").team === "爪牙"
                        ? "；目标为爪牙，但尚未确认实际死亡，不授予保留能力"
                      : "")
    ...
    if (sent && selectedPlayer?.roleId === "lunatic") {
      const trueDemons = state.players.filter(
        (player) =>
          player.id !== selectedPlayer.id &&
          getRole(player.roleId).team === "恶魔",
      );
      for (const demon of trueDemons) {
        try {
          await onSendMessage({
            playerId: demon.id,
            roleId: demon.roleId,
            body: `疯子${formatSeat(selectedPlayer.seat)}本晚选择：${targetLabel}`,
          });
        } catch {
          setSendError(`疯子的选择已记录，但未能转告${formatSeat(demon.seat)}恶魔`);
        }
      }
    }
    if (
      ...
      currentRole.id === "pukka" &&
      selectedAbilityIssueLabels.length === 0 &&
      previousPukkaTarget?.alive
    ) {
      const died = await onSetPlayerAlive(previousPukkaTarget.id, false);
      if (!died) {
        setSendError("上一名中毒者受到死亡保护，本晚没有死亡");
      }
    }
    if (sent && currentRole.id === "nodashii") {
      persistNodashiiPoisonTargets();
    }
    if (sent && vigormortisKillsMinion && vigormortisTarget) {
      const retainedNote: PlayerNoteEntry = {
        id: `${vigormortisRetainedAbilityNoteId}:${selectedPlayer?.id ?? "unknown"}`,
      ...
      onUpdate({
        players: state.players.map((player) =>
          player.id === vigormortisTarget.id
            ? {
                ...player,
                alive: false,
                notes: serializePlayerNotes([
                  ...
                  ...parsePlayerNotes(player.notes).filter(
                    (note) =>
                      !note.id.startsWith(vigormortisRetainedAbilityNoteId),
                  ),
                ]),
              }
            : player,
    ...
    onUpdate({
      players: state.players.map((player) => {
        ...
        if (impSuccessor && player.id === impSuccessor.id) {
          return {
            ...player,
            roleId: "imp",
            drunkRoleId: "",
  ...
  const sendNodashiiPoisonSetup = async () => {
    if (!isNodashiiFirstNight || !currentRole || !selectedPlayer) return;
    const poisonedSeats = nodashiiPoisonedPlayers.map((player) =>
      formatSeat(player.seat),
    );
    if (
      !window.confirm(
        `确认把${poisonedSeats.join("、") || "未找到镇民"}标记为诺-达鲷持续中毒？\n\n这只写入主持人魔典，不会通知诺-达鲷玩家。`,
      )
    ) return;
    persistNodashiiPoisonTargets();
    setSendError("");
  ...
  const sendGodfatherOutsiderInfo = async () => {
    if (currentRole?.id !== "godfather") return;
    const selectedRoleIds = [...godfatherOutsiderRoleIds].sort();
    const actualRoleIds = [...actualOutsiderRoleIds].sort();
    const omittedActualRoles = actualRoleIds.filter(
      (roleId) => !selectedRoleIds.includes(roleId),
    );
    const registeredExtras = selectedRoleIds.filter(
      (roleId) => !actualRoleIds.includes(roleId),
    );
    const spyCount = state.players.filter((player) => player.roleId === "spy").length;
    const matchesRegistrationRules =
      omittedActualRoles.length === 0 && registeredExtras.length <= spyCount;
    const selectedNames = selectedRoleIds.map((roleId) => getRole(roleId).name);
    const actualNames = actualRoleIds.map((roleId) => getRole(roleId).name);
    if (!(await confirmSkillResult({
      title: "确认发送教父首夜信息？",
      sendLines: [
        selectedNames.length
          ? `发送在场外来者：${selectedNames.join("、")}`
          : "发送：本局没有在场外来者",
      ],
      sentMatchesBaseline: matchesRegistrationRules,
      baseline: actualNames.length
        ? `真实在场外来者：${actualNames.join("、")}`
        : "真实在场外来者：无",
      caveats: spyCount
        ? [`场上有 ${spyCount} 名间谍，可各自额外登记为一个外来者角色`]
        : [],
    }))) return;
    void submitSkill(
      selectedNames.length
        ? `本局在场外来者：${selectedNames.join("、")}`
        : "本局没有在场外来者",
    );
  };
  ...
  const sendNightwatchmanNotice = async () => {
    ...
    if (selectedAbilityIssueLabels.length) {
      await submitSkill(
        `你选择了${getNightSeatLabel(singleSkillTargetId)}，但本晚能力未生效`,
      );
      return;
    }
    const target = state.players.find(
  ...
  const sendRoleRevealSkill = async () => {
    ...
    if (!(await confirmSkillResult({
      ...
      sentMatchesBaseline: target
        ? canRegisterAsRole(target.roleId, revealedSkillRoleId, getRole)
        : false,
      sentMatchesStrictBaseline: target?.roleId === revealedSkillRoleId,
      baseline: target
  ...
  const sendScarletWomanResult = async (triggered: boolean) => {
    const aliveCount = state.players.filter((player) => player.alive).length;
    const aliveBeforeDemonDeath = aliveCount + 1;
    const deadRecluses = state.players.filter(
      (player) => !player.alive && player.roleId === "recluse",
    );
    const inheritedDemon = getRole(scarletDemonRoleId);
    const abilityHasEffect = selectedAbilityIssueLabels.length === 0;
    const trueBaselineTriggered =
      aliveBeforeDemonDeath >= 5 && deadDemonPlayers.length > 0;
    const recluseCouldTrigger =
      aliveBeforeDemonDeath >= 5 && deadRecluses.length > 0;
    const resultIsCompatible =
      !abilityHasEffect ||
      (trueBaselineTriggered
        ? triggered
        : recluseCouldTrigger
          ? true
          : !triggered);
    if (triggered && inheritedDemon.team !== "恶魔") return;
    if (!(await confirmSkillResult({
      title: "确认发送红唇女郎继承结果？",
      sendLines: [
        `发送结果：${triggered ? `已继承${inheritedDemon.name}能力` : "本晚未触发恶魔继承"}`,
      ],
      sentMatchesBaseline: resultIsCompatible,
      baseline: `${aliveCount} 名玩家当前存活，按恶魔死亡前计为 ${aliveBeforeDemonDeath} 名；${
        deadDemonPlayers.length
          ? `已死亡的真实恶魔为${deadDemonPlayers
              .map((player) => `${formatSeat(player.seat)} ${getRole(player.roleId).name}`)
              .join("、")}`
          : "没有真实恶魔死亡"
      }，按真实身份基准${trueBaselineTriggered ? "应触发继承" : "不应触发继承"}`,
    ...
    if (
      ...
      !triggered ||
      !selectedPlayer ||
      selectedPlayer.roleId !== "scarlet-woman"
    ) return;
    onUpdate({
      players: state.players.map((player) =>
        player.id === selectedPlayer.id
          ? {
              ...player,
              roleId: inheritedDemon.id,
              drunkRoleId: "",
              identityMessage: `你已继承${inheritedDemon.name}身份。`,
            }
          : player,
      ),
    });
  ...
  const sendGrandmotherInfo = async () => {
    ...
    const sent = await submitSkill(
      `你的孙辈是${targetLabel}，角色是${shownRole.name}`,
    );
    if (!sent || !selectedPlayer || !target) return;
    const grandchildNote: PlayerNoteEntry = {
      id: `${grandmotherGrandchildNotePrefix}${target.id}`,
      body: `祖母孙辈：${targetLabel} ${shownRole.name}`,
      createdAt: new Date().toISOString(),
      stage: currentStageLabel,
    };
    onUpdate({
        ...
        player.id === selectedPlayer.id
          ? {
              ...
              notes: serializePlayerNotes([
                grandchildNote,
                ...parsePlayerNotes(player.notes).filter(
                  (note) => !note.id.startsWith(grandmotherGrandchildNotePrefix),
                ),
              ]),
            }
          : player,
  ...
  const selectRandomGrandchild = () => {
    ...
    const goodRoles = getScriptRoles(state.scriptId).filter((role) =>
      ["镇民", "外来者"].includes(role.team),
    );
    const shownRoleId = randomGrandchild.roleId === "spy"
      ? goodRoles[Math.floor(Math.random() * goodRoles.length)]?.id ?? ""
      : randomGrandchild.roleId;
    setSingleSkillTargetId(randomGrandchild.id);
    setRevealedSkillRoleId(shownRoleId);
  };

  const resolveGrandmotherDeath = async (triggered: boolean) => {
    const sent = await submitSkill(
      selectedAbilityIssueLabels.length
        ? "本晚祖母能力失效，未触发同死"
        : triggered
        ? "孙辈被恶魔杀死，祖母同时死亡"
        : "本晚孙辈未被恶魔杀死",
    ...
    if (
      ...
      await onSetPlayerAlive(selectedPlayer.id, false);
    }
  };
  ...
  const sendFortuneTellerResult = async (hasDemon: boolean) => {
    ...
    if (!(await confirmSkillResult({
      ...
      sentMatchesBaseline: hasDemon === baselineHasDemon || (!baselineHasDemon && hasDemon && recluseTargets.length > 0),
      sentMatchesStrictBaseline: hasDemon === baselineHasDemon,
  ...
  const sendChambermaidResult = async (reportedCount: number) => {
    ...
    const assessments = targets.map((player) => {
      ...
      const disguisedAbilityRoleId =
        ...
        player.drunkRoleId
          ...
          : "";
      const abilityRoleId =
        philosopherAbilityRoleId || disguisedAbilityRoleId || player.roleId;
    ...
    if (!(await confirmSkillResult({
      ...
      sentMatchesStrictBaseline:
        definiteWakeCount === maximumWakeCount
          ? reportedCount === definiteWakeCount
          : null,
      baseline,
  ...
  const sendJugglerResult = async (count: number) => {
    ...
    if (
      !(await confirmSkillResult({
        ...
        sentMatchesBaseline: jugglerResultOptions.includes(count),
        sentMatchesStrictBaseline: count === calculatedJugglerResult,
        baseline: `${detail}；按登记规则可猜对 ${jugglerResultOptions.join("、")} 个（按真实身份为 ${calculatedJugglerResult} 个）`,
  ...
  const sendOracleResult = async (count: number) => {
    ...
    if (!(await confirmSkillResult({
      ...
      sentMatchesStrictBaseline: count === oracleAssessment.trueEvilCount,
  ...
  const sendGamblerResult = async (reportedCorrect: boolean) => {
    ...
    const correctOptions = registrationCompatible
      ? target && ["recluse", "spy"].includes(target.roleId)
        ? [false, true]
        : [true]
      : [false];
    ...
    if (!(await confirmSkillResult({
      ...
      sentMatchesBaseline: correctOptions.includes(reportedCorrect),
      baseline: target
        ? `${targetLabel}的真实身份是${actualRole.name}，按真实身份猜测${baselineCorrect ? "正确" : "错误"}；按登记规则可判为${correctOptions.map((option) => option ? "正确" : "错误").join("或")}`
        : "未找到玩家选择的目标",
  ...
  const sendAlignmentResult = async (reportedGood: boolean) => {
    ...
    const targetLabel = target ? formatSeat(target.seat) : "未知目标";
    const outcome = !abilityHasEffect
      ? "能力失效，目标不会因此死亡或导致阵营落败"
      : currentRole.id === "moonchild"
      ? reportedGood
        ? "目标为善良，今晚死亡"
        : "目标不按善良判定，不会因此死亡"
      : reportedGood
    ...
    if (!(await confirmSkillResult({
      ...
      sentMatchesBaseline: !abilityHasEffect || (target
        ? reportedGood
          ? canRegisterAsGood(target.roleId, actualRole.team)
          : canRegisterAsEvil(target.roleId, actualRole.team)
        : false),
      baseline: target
    ...
    const sent = await submitSkill(
      currentRole.id === "moonchild"
        ? `${latestPlayerSkillChoice?.summary ?? "选择目标"}：能力已完成夜间结算`
        : `${latestPlayerSkillChoice?.summary ?? "选择目标"}：${outcome}`,
    );
    if (
      sent &&
      abilityHasEffect &&
  ...
  const sendTownCrierResult = async (reportedYes: boolean) => {
    const nominators = townCrierNominators.length
      ? townCrierNominators
          .map((player) => `${formatSeat(player.seat)} ${getRole(player.roleId).name}`)
          .join("、")
      : "今天无人发起提名";
    if (!(await confirmSkillResult({
      title: "确认发送城镇公告员结果？",
      sendLines: [`发送结果：${reportedYes ? "是" : "否"}`],
      sentMatchesBaseline: townCrierResultOptions.has(reportedYes),
      sentMatchesStrictBaseline: reportedYes === townCrierActualYes,
      baseline: `当天提名者：${nominators}；按真实身份应为${townCrierActualYes ? "是" : "否"}`,
      caveats: townCrierNominators.some((player) =>
        ["recluse", "spy", "marionette"].includes(player.roleId),
      )
        ? ["陌客、间谍或提线木偶可能改变本次爪牙登记"]
        : [],
    }))) return;
    void submitSkill(`今天${reportedYes ? "有" : "没有"}爪牙发起提名`);
  };

  const sendSeamstressResult = async (reportedSame: boolean) => {
    if (!pairTargetsReady) return;
    const targets = [skillTargets.first, skillTargets.second]
      .map((playerId) => state.players.find((player) => player.id === playerId))
      .filter((player): player is Player => Boolean(player));
    if (targets.length !== 2) return;
    const actualSame =
      ["镇民", "外来者"].includes(getRole(targets[0].roleId).team) ===
      ["镇民", "外来者"].includes(getRole(targets[1].roleId).team);
    const flexible = targets.some((player) =>
      ["recluse", "spy"].includes(player.roleId),
    );
    if (!(await confirmSkillResult({
      title: "确认发送女裁缝结果？",
      sendLines: [
        `查验目标：${getNightSeatLabel(targets[0].id)}、${getNightSeatLabel(targets[1].id)}`,
        `发送结果：阵营${reportedSame ? "相同" : "不同"}`,
      ],
      sentMatchesBaseline: flexible || reportedSame === actualSame,
      sentMatchesStrictBaseline: reportedSame === actualSame,
      baseline: `${formatSeat(targets[0].seat)}为${getRole(targets[0].roleId).team}，${formatSeat(targets[1].seat)}为${getRole(targets[1].roleId).team}；按真实阵营应为${actualSame ? "相同" : "不同"}`,
      caveats: flexible ? ["陌客或间谍可能改变善恶阵营登记"] : [],
    }))) return;
    void submitSkill(
      `你选择的${getNightSeatLabel(targets[0].id)}和${getNightSeatLabel(targets[1].id)}阵营${reportedSame ? "相同" : "不同"}`,
    );
  };

  const resolveGossip = async (statementIsTrue: boolean) => {
    if (!latestPlayerSkillChoice?.statement) {
      setSendError("造谣者尚未提交今天的公开声明");
      return;
    }
    if (
      statementIsTrue &&
      ...
      setSendError("请选择一名玩家结算额外死亡");
      return;
    }
    if (
      !window.confirm(
        `公开声明：${latestPlayerSkillChoice.statement}\n\n确认按“${statementIsTrue ? "真实" : "虚假"}”结算？`,
      )
    ) return;
    const sent = await submitSkill("你今天的公开声明已完成夜间结算");
    if (
      sent &&
      statementIsTrue &&
      selectedAbilityIssueLabels.length === 0
      ...
      if (target?.alive) {
        const died = await onSetPlayerAlive(target.id, false);
        if (!died) setSendError("目标受到死亡保护，本次额外死亡未发生");
      }
    }
  };

  const resolveBarberSwap = async (swap: boolean) => {
    if (!selectedPlayer || currentRole?.id !== "barber") return;
    const first = state.players.find((player) => player.id === skillTargets.first);
    const second = state.players.find((player) => player.id === skillTargets.second);
    if (swap && (!first || !second || first.id === second.id)) return;
    if (
      swap &&
      (!barberSwapCandidates.some((player) => player.id === first?.id) ||
        !barberSwapCandidates.some((player) => player.id === second?.id))
    ) {
      setSendError("理发师交换不能选择另一名恶魔");
      return;
    }
    const description = swap && first && second
      ? `${formatSeat(first.seat)} ${getRole(first.roleId).name}与${formatSeat(second.seat)} ${getRole(second.roleId).name}交换角色`
      : "恶魔放弃理发师角色交换";
    if (!window.confirm(`确认${description}？`)) return;
    const sent = room
      ? await submitSkill("理发师能力已完成结算")
      : true;
    if (!sent) return;
    onUpdate({
      players: state.players.map((player) => {
        const notes = parsePlayerNotes(player.notes).map((note) =>
          player.id === selectedPlayer.id &&
          note.id.startsWith(barberDeathNotePrefix) &&
          !note.resolved
            ? { ...note, resolved: true }
        ...
        if (!swap || !first || !second) {
          return { ...player, notes: serializePlayerNotes(notes) };
        }
        if (player.id === first.id) {
          return {
            ...player,
            roleId: second.roleId,
            drunkRoleId: second.drunkRoleId,
            identityMessage: `你的角色已因理发师能力变为${getRole(second.roleId).name}。`,
            notes: serializePlayerNotes(notes),
          };
        }
        if (player.id === second.id) {
          return {
            ...player,
            roleId: first.roleId,
            drunkRoleId: first.drunkRoleId,
            identityMessage: `你的角色已因理发师能力变为${getRole(first.roleId).name}。`,
            notes: serializePlayerNotes(notes),
          };
        }
        return { ...player, notes: serializePlayerNotes(notes) };
    ...
    <div className="night-layout">
      <section className="night-main">
        <section className="night-action-section">
          ...
          {poisonedPlayerStatuses.length ? (
            <div className="night-persistent-status" role="status">
              <Biohazard size={17} />
              <strong>当前中毒</strong>
              <div>
                {poisonedPlayerStatuses.map(({ player, mark }) => (
                  <span key={player.id}>
                    {formatSeat(player.seat)} {getRole(player.roleId).name}
                ...
                return (
                  <button
                    className={[
                      ...
                      isDead ? "is-dead" : "",
                      action.setupOnly ? "is-setup-only" : "",
                  ...
                  >
                    ...
                    <span className="night-role-team">
                      {action.setupOnly ? "主持设置" : action.role.team}
                    </span>
                    {state.nightIndex === index ? (
                      <span className="on-air">
                        {action.setupOnly ? "设置中" : "进行中"}
                      </span>
                    ) : null}
                  </button>
                );
        ...
        {currentRole ? (
          <>
              ...
              <div className="night-current-role-header">
                <div className="night-current-role-heading">
                  ...
                  <div>
                    ...
                    <h3>当前行动 · {currentAction?.name ?? currentRole.name}</h3>
                ...
                {currentAction?.setupOnly ? (
                  <div className="night-host-only-action">
                    <BookOpen size={16} />
                    <span>
                      <strong>主持人设置</strong>
                      <small>不唤醒玩家，也不向诺-达鲷发送邻位身份</small>
                    </span>
                  </div>
            ...
            <div className="night-skill-composer">
                ...
                <div className="night-skill-panel-heading">
                  ...
                  {currentRole.id === "librarian" || currentRole.id === "investigator" ? (
                    <div className="night-skill-mode">
                      ...
                      >
                        展示{currentRole.id === "librarian" ? "外来者" : "爪牙"}
                      </button>
                      <button
                        className={librarianNoOutsider ? "active" : ""}
                        onClick={() => setLibrarianNoOutsider(true)}
                      >
                        没有{currentRole.id === "librarian" ? "外来者" : "爪牙"}
                      </button>
                    </div>
                  ) : null}
                  {!noPairRoleInPlay ? (
                  ...
                  >
                    ...
                    {sendingMode === "skill"
                      ? "发送中"
                      : noPairRoleInPlay
                        ? `发送“没有${currentRole.id === "librarian" ? "外来者" : "爪牙"}”`
                        : "发送技能信息"}
                  </button>
                </div>
              ) : null}
              {currentRole.id === "chef" ? (
                <div className="night-skill-panel compact">
                  <div className="night-skill-panel-heading">
                    <div className="night-skill-heading-title">
                      <span><ScrollText size={14} />厨师结果</span>
                      <small>{currentRole.short}</small>
                    </div>
                  </div>
                  <div className="oracle-summary">
                    <article>
                      <span>真实阵营</span>
                      <strong>{calculatedChefResult}</strong>
                      <small>按当前真实身份计算</small>
                    </article>
                    <article className="registration">
                      <span>可登记结果</span>
                      <strong>{chefResultOptions.join("、")}</strong>
                      <small>已考虑陌客与间谍</small>
                    </article>
                  </div>
                  <div className="oracle-result-editor">
                    <label>
                      <span>准备发送的相邻邪恶对数</span>
                      <div className="oracle-number-field">
                        <button
                          type="button"
                          disabled={!canUseSkill || chefResult <= 0}
                          onClick={() => setChefResult((value) => Math.max(0, value - 1))}
                          aria-label="减少厨师结果"
                        >−</button>
                        <input
                          type="number"
                          ...
                          aria-label="厨师发送的相邻邪恶对数"
                        />
                        <button
                          type="button"
                          disabled={!canUseSkill}
                          onClick={() => setChefResult((value) => value + 1)}
                          aria-label="增加厨师结果"
                        >+</button>
                      </div>
                    </label>
              ...
              {isMinionInfoAction ? (
                <div className="night-skill-panel compact">
                  <div className="night-skill-panel-heading">
                    <div className="night-skill-heading-title">
                      <span><Users size={14} />爪牙阵营信息</span>
                      <small>向所有已入座且会被唤醒的爪牙发送恶魔与爪牙同伴的座位和身份</small>
                    </div>
                  </div>
                  <div className="night-player-choice"><Skull size={14} /><span>恶魔身份：{demonIdentitySummary}</span></div>
                  <div className="night-player-choice"><Users size={14} /><span>会被唤醒的爪牙：{awakeMinionIdentitySummary}</span></div>
                  <button className="primary-button night-skill-submit" disabled={!room || sending} onClick={() => void sendMinionInfo()}>
                    <Send size={15} />发送完整爪牙信息
                  </button>
                </div>
              ) : null}
              {isDemonInfoAction && !isLunaticDemonInfo ? (
                <div className="night-skill-panel demon-bluff-draft">
                  ...
                  <div className="night-player-choice"><Users size={14} /><span>{demonMinionInfoBody}</span></div>
                  {demonBluffRoleIds.length === 3 ? (
                  ...
                  <button className="primary-button demon-bluff-send" disabled={demonInfoAlreadySent || sendingBluffs || !canUseSkill || demonBluffRoleIds.length !== 3} onClick={() => void sendDemonBluffs()}>
                    {demonInfoAlreadySent ? <Check size={15} /> : <Send size={15} />}
                    {demonInfoAlreadySent ? "完整信息已发送" : sendingBluffs ? "发送中" : selectedRoomPlayer?.is_claimed ? "发送完整恶魔信息" : `${currentRole.name}入座后可发送`}
                  </button>
                </div>
              ) : null}
              {isLunaticDemonInfo ? (
                ...
                <div className="night-skill-panel">
                  ...
                  {currentRole.id === "imp" && singleSkillTargetId === selectedPlayer?.id ? (
                    impSuccessorCandidates.length ? (
                      ...
                      <div className="night-player-choice waiting"><Target size={14} /><span>没有存活爪牙，自杀后无人继承恶魔身份</span></div>
                    )
                  ) : null}
                  <div className="night-skill-single-row">
                    ...
                    <button className="primary-button" disabled={!canUseSingleTargetSkill || !singleSkillTargetId || (currentRole.id === "imp" && singleSkillTargetId === selectedPlayer?.id && impSuccessorCandidates.length > 0 && !impSuccessorPlayerId)} onClick={() => void sendSingleTargetSkill()}>
              ...
              {currentRole.id === "scarlet-woman" ? (
                <div className="night-skill-panel compact">
                  <div className="night-skill-panel-heading">
                    <div className="night-skill-heading-title">
                      <span><Skull size={14} />红唇女郎继承</span>
                      <small>恶魔死亡前至少五人存活时，继承死亡恶魔的角色与能力</small>
                    </div>
                  </div>
                  <label className="night-skill-role-field">
                    <span>继承恶魔角色</span>
                    <CompactSelect
                      value={scarletDemonRoleId}
                      disabled={sending}
                      onChange={(event) => setScarletDemonRoleId(event.target.value)}
                    >
                      {scriptDemonRoles.map((role) => (
                  ...
                  <div className="night-skill-result-grid fortune-results">
                    ...
                    <button className="secondary-button positive" disabled={!canUseSkill || !scarletDemonRoleId} onClick={() => sendScarletWomanResult(true)}>确认继承并更新魔典</button>
                  </div>
                </div>
              ) : null}
              {currentRole.id === "grandmother" ? (
                <div className="night-skill-panel">
                  ...
                  {state.round <= 1 || philosopherAbilityJustAcquired ? (
                    <>
                      <div className="night-skill-reveal-grid">
                    ...
                    <div className="night-skill-result-grid fortune-results">
                      <button className="secondary-button" disabled={!canUseSkill} onClick={() => void resolveGrandmotherDeath(false)}>未触发</button>
                      <button className="secondary-button positive" disabled={!canUseSkill} onClick={() => void resolveGrandmotherDeath(true)}>祖母同死</button>
                    </div>
                  )}
                </div>
              ) : null}
              ...
              {currentRole.id === "town-crier" ? (
                <div className="night-skill-panel compact">
                  <div className="night-skill-panel-heading"><div className="night-skill-heading-title"><span><ScrollText size={14} />城镇公告员结果</span><small>核对上一白天是否有爪牙发起过提名</small></div></div>
                  <div className="night-player-choice"><Users size={14} /><span>{townCrierNominators.length ? `当天提名者：${townCrierNominators.map((player) => `${formatSeat(player.seat)} ${getRole(player.roleId).name}`).join("、")}` : "当天没有人发起提名"}</span></div>
                  <div className="night-skill-result-grid fortune-results">
                    <button className="secondary-button positive" disabled={!canUseSkill} onClick={() => void sendTownCrierResult(true)}>是<small>有爪牙提名</small></button>
                    <button className="secondary-button negative" disabled={!canUseSkill} onClick={() => void sendTownCrierResult(false)}>否<small>没有爪牙提名</small></button>
                  </div>
                </div>
              ) : null}
              {currentRole.id === "seamstress" ? (
                <div className="night-skill-panel compact">
                  <div className="night-skill-panel-heading"><div className="night-skill-heading-title"><span><Target size={14} />女裁缝查验</span><small>{currentRole.short}</small></div></div>
                  <div className={latestPlayerSkillChoice ? "night-player-choice" : "night-player-choice waiting"}><Target size={14} /><span>{latestPlayerSkillChoice ? `玩家已提交：${latestPlayerSkillChoice.summary}` : "等待玩家选择两名其他玩家"}</span></div>
                  <div className="night-skill-target-grid">
                    <CompactSelect value={skillTargets.first} onChange={(event) => setSkillTargets((current) => ({ ...current, first: event.target.value }))}>{singleTargetCandidates.map((player) => <option value={player.id} key={player.id} disabled={player.id === skillTargets.second}>{getNightPlayerLabel(player.id)}</option>)}</CompactSelect>
                    <CompactSelect value={skillTargets.second} onChange={(event) => setSkillTargets((current) => ({ ...current, second: event.target.value }))}>{singleTargetCandidates.map((player) => <option value={player.id} key={player.id} disabled={player.id === skillTargets.first}>{getNightPlayerLabel(player.id)}</option>)}</CompactSelect>
                  </div>
                  <div className="night-skill-result-grid fortune-results">
                    <button className="secondary-button positive" disabled={!canUseSkill || !pairTargetsReady} onClick={() => void sendSeamstressResult(true)}>相同</button>
                    <button className="secondary-button negative" disabled={!canUseSkill || !pairTargetsReady} onClick={() => void sendSeamstressResult(false)}>不同</button>
                  </div>
                </div>
              ) : null}
              {currentRole.id === "gossip" ? (
                <div className="night-skill-panel compact">
                  <div className="night-skill-panel-heading"><div className="night-skill-heading-title"><span><ScrollText size={14} />造谣声明结算</span><small>{currentRole.short}</small></div></div>
                  <div className={latestPlayerSkillChoice?.statement ? "night-player-choice" : "night-player-choice waiting"}><MessageSquareText size={14} /><span>{latestPlayerSkillChoice?.statement ?? "玩家尚未提交今天的公开声明"}</span></div>
                  <label className="night-skill-role-field"><span>声明为真时额外死亡</span><CompactSelect value={singleSkillTargetId} onChange={(event) => setSingleSkillTargetId(event.target.value)}>{singleTargetCandidates.map((player) => <option value={player.id} key={player.id}>{getNightPlayerLabel(player.id)}</option>)}</CompactSelect></label>
                  <div className="night-skill-result-grid fortune-results">
                    <button className="secondary-button negative" disabled={!canUseSkill || !latestPlayerSkillChoice?.statement} onClick={() => void resolveGossip(false)}>声明为假</button>
                    <button className="secondary-button positive" disabled={!canUseSkill || !latestPlayerSkillChoice?.statement || !singleSkillTargetId} onClick={() => void resolveGossip(true)}>声明为真·结算死亡</button>
                  </div>
                </div>
              ) : null}
              {currentRole.id === "barber" ? (
                <div className="night-skill-panel compact">
                  <div className="night-skill-panel-heading"><div className="night-skill-heading-title"><span><RotateCcw size={14} />理发师角色交换</span><small>由{barberDemon ? `${formatSeat(barberDemon.seat)} ${getRole(barberDemon.roleId).name}` : "存活恶魔"}决定</small></div></div>
                  {!barberDemon ? <div className="inline-error">没有存活恶魔可以发动理发师能力</div> : null}
                  <div className="night-skill-target-grid">
                    <CompactSelect value={skillTargets.first} onChange={(event) => setSkillTargets((current) => ({ ...current, first: event.target.value }))}>{barberSwapCandidates.map((player) => <option value={player.id} key={player.id} disabled={player.id === skillTargets.second}>{getNightPlayerLabel(player.id)}</option>)}</CompactSelect>
                    <CompactSelect value={skillTargets.second} onChange={(event) => setSkillTargets((current) => ({ ...current, second: event.target.value }))}>{barberSwapCandidates.map((player) => <option value={player.id} key={player.id} disabled={player.id === skillTargets.first}>{getNightPlayerLabel(player.id)}</option>)}</CompactSelect>
                  </div>
                  <div className="night-skill-result-grid fortune-results">
                    <button className="secondary-button negative" disabled={!canResolveBarber} onClick={() => void resolveBarberSwap(false)}>放弃交换</button>
                    <button className="secondary-button positive" disabled={!canResolveBarber || !barberDemon || !pairTargetsReady} onClick={() => void resolveBarberSwap(true)}>确认交换角色</button>
                  </div>
                </div>
              ) : null}
              ...
              {currentRole.id === "godfather" && state.round <= 1 ? (
                <div className="night-skill-panel compact">
                  ...
                  <div className="demon-bluff-draft-roles">
                    {scriptOutsiderRoles.map((role) => (
                      <label className="night-player-choice" key={role.id}>
                        <input
                          type="checkbox"
                          checked={godfatherOutsiderRoleIds.includes(role.id)}
                          onChange={(event) =>
                            setGodfatherOutsiderRoleIds((current) =>
                  ...
                  {state.players.some((player) => player.roleId === "spy") ? <small>真实外来者已默认勾选；可额外勾选间谍本次登记的外来者身份。</small> : null}
                ...
                <div className="night-skill-panel">
                  ...
                  {currentRole.id === "pukka" ? (
                    <div className="night-player-choice">
                      ...
                      <span>
                        {previousPukkaTarget
                          ? `上一名中毒者：${getNightPlayerLabel(previousPukkaTarget.id)}；先放置新中毒，再结算该玩家死亡并恢复健康`
                          : state.round <= 1
                            ? "首夜没有上一名中毒者，只选择新的中毒目标"
                            : "未找到上一晚的普卡中毒记录，请手动核对后再发送"}
                      </span>
                    </div>
                  ) : null}
                  {currentRole.id === "godfather" ? (
                  ...
                  {!isNodashiiFirstNight ? <div className={latestPlayerSkillChoice ? "night-player-choice" : "night-player-choice waiting"}><Target size={14} /><span>{latestPlayerSkillChoice ? `玩家已提交：${latestPlayerSkillChoice.summary}` : currentRole.id === "godfather" ? godfatherRevengeNotified ? "已通知教父，等待玩家选择目标" : "仅在白天有外来者死亡时通知教父" : "玩家尚未提交，可由上帝代选"}</span></div> : null}
                  {currentRole.id === "nodashii" ? <div className="night-player-choice"><Biohazard size={14} /><span>当前中毒镇民：{nodashiiPoisonedPlayers.map((player) => `${formatSeat(player.seat)} ${getRole(player.roleId).name}`).join("、") || "未找到"}</span></div> : null}
                  {isNodashiiFirstNight ? <div className="night-host-only-note"><BookOpen size={14} /><span>仅保存到主持人魔典；不会唤醒玩家，也不会把中毒座位发送给诺-达鲷。</span></div> : null}
                  {["nodashii", "vigormortis"].includes(currentRole.id) && state.players.some((player) => player.roleId === "spy") ? <label className="night-player-choice"><input type="checkbox" checked={registerSpyAsTownsfolkForAdjacency} onChange={(event) => setRegisterSpyAsTownsfolkForAdjacency(event.target.checked)} /><span>本次邻接判定将间谍登记为镇民</span></label> : null}
                  {currentRole.id === "vigormortis" && getRole(state.players.find((player) => player.id === singleSkillTargetId)?.roleId ?? "").team === "爪牙" ? <><label className="night-player-choice"><input type="checkbox" checked={vigormortisKillConfirmed} onChange={(event) => setVigormortisKillConfirmed(event.target.checked)} /><span>确认该爪牙实际因本次攻击死亡，并在死亡后保留能力</span></label>{vigormortisKillConfirmed ? <label className="night-skill-role-field"><span>中毒镇民</span><CompactSelect value={skillTargets.second} onChange={(event) => setSkillTargets((current) => ({...current, second: event.target.value}))}>{vigormortisPoisonCandidates.map((player) => <option value={player.id} key={player.id}>{getNightPlayerLabel(player.id)}</option>)}</CompactSelect></label> : null}</> : null}
                  {isNodashiiFirstNight ? <button className="primary-button night-skill-submit" disabled={!currentAction?.canAct || sending || !nodashiiPoisonedPlayers.length} onClick={() => void sendNodashiiPoisonSetup()}><Check size={15} />保存主持标记</button> : <div className="night-skill-single-row"><CompactSelect value={singleSkillTargetId} disabled={currentRole.id === "godfather" && !latestPlayerSkillChoice} onChange={(event) => setSingleSkillTargetId(event.target.value)}>{singleTargetCandidates.map((player) => <option value={player.id} key={player.id}>{getNightPlayerLabel(player.id)}</option>)}</CompactSelect><button className="primary-button" disabled={!canUseSingleTargetSkill || !singleSkillTargetId || (currentRole.id === "godfather" && (!godfatherRevengeNotified || !latestPlayerSkillChoice))} onClick={sendSingleTargetSkill}><Send size={15} />确认并发送</button></div>}
                </div>
              ) : null}
              {currentRole.id === "marionette" ? (
                <div className="night-skill-panel compact">
            ...
            {!currentAction?.setupOnly ? <section className="night-message-chat">
