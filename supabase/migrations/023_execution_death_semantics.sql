alter table public.xueran_day_resolutions
  add column if not exists executed_player_was_alive boolean;

-- Historical rows predate this distinction; preserving the former behavior is
-- safer than silently suppressing already-recorded Undertaker/Saint results.
update public.xueran_day_resolutions
set executed_player_was_alive = true
where executed_player_id is not null
  and executed_player_was_alive is null;
...
begin
  ...
  select executed_player_id, executed_player_was_alive
  into existing_execution, existing_execution_was_alive
